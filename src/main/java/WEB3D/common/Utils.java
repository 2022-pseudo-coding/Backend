...
public class Utils {
    ...
    public static List<Status> execInstructionsWithModule(Problem problem, List<Instruction> instructions,Long userId) throws ExecInstructionException {
        List<Status> statusList = new ArrayList<>();
        List<Instruction> available_inst = problem.getInstructions();
        for (Instruction instruction : instructions) {
            boolean found = false;
        ...
        while (!finish && !exception && inst_index < instructions.size()) {
            Instruction instruction = instructions.get(inst_index);
            Module findModule=moduleRepository.findByCreatorIdAndName(userId,instruction.getName());
            List<Instruction> instructionList=new ArrayList<>();
            if(findModule!=null){
                instructionList.addAll(findModule.getInstructions());
            }else{
